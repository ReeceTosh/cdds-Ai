...
void EntityEditorApp::Update(float deltaTime) {
	...
	static bool sizeEditMode = true;
	static bool speedEditMode = true;
	...
	char* data = (char*)MapViewOfFile(fileHandle, FILE_MAP_ALL_ACCESS, 0, 0, sizeof(m_entities) + sizeof(size_t));
	if (data)
	{
		size_t entityCount = ENTITY_COUNT;
		//sharing size of x number of entities before draw
		memcpy(data, &entityCount, sizeof(size_t));
		//sharing the data of the entities
		memcpy(data + sizeof(size_t), m_entities, sizeof(m_entities));
...
void EntityEditorApp::CreateFileMappingEntities()
{
